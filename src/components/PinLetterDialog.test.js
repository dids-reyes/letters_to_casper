...
test('rejects an invalid link without making a request', () => {
  ...
  fireEvent.click(screen.getByText('Pin for ₱29'));
...
test('previews the letter before creating a checkout and sends the selected tier and optional email', async () => {
  ...
  fireEvent.click(screen.getByText('Pin for ₱89'));
  ...
  expect(fetch).toHaveBeenLastCalledWith('https://service.example/api/create-pin-payment', expect.objectContaining({body: JSON.stringify({letterId: id, tierId: '3d', customer_email: 'reader@example.com'})}));
...
test('unavailable letters never reach confirmation or checkout', async () => {
  ...
  render(<PinLetterDialog onClose={() => {}} />); fillLink(); fireEvent.click(screen.getByText('Pin for ₱29'));
  await screen.findByRole('alert');
  expect(screen.queryByText('Proceed to Payment')).toBeNull();
  expect(screen.getByText('Pin for ₱29').disabled).toBe(false);
});
...
test('starts with an empty link field and defaults to 12 hours (₱29)', () => {
